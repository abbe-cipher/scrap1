            ...
            <script>
                async function startScraping() {
                    ...
                    try {
                        ...
                        document.getElementById('endTime').innerText = 'End Time: ' + data.endTime + ' (Duration: ' + data.timeSpentSec + ' seconds)';
...
app.get('/scrape', async (req, res) => {
    ...
    // Wait for the specific element that contains the score
    ...
    const timeSpentSec = ((endTimeMs - startTimeMs) / 1000).toFixed(2); // Calculate time in seconds, fixed to 2 decimal places

    // Send the results back as JSON
    res.json({
        startTime,
        endTime,
        timeSpentSec, // Include the time spent
